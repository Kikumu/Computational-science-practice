...
def region_of_interest(width,height):
    vertices = [(0,height),(0,height/2),(width/2,height/2),(width,height/2),(width,height)]
    return vertices
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    #image = draw_lines(image,scrn_x,scrn_y)
    vertices = region_of_interest(640,480)
    
    
    image = ROI_mask(image,np.array([vertices],np.int32),)
